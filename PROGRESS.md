## 2026-07-29 (집) — fix: 작업내역 중복 저장 버그 수정
- 브랜치: main
- 완료:
  - **작업내역 2개 생성 버그 수정**: handleAutoAll에서 업로드 완료 시 `POST /api/sheets`(새 레코드) + `PUT /api/pipeline/job`(기존 업데이트) 동시 실행 → 중복. jobId 있으면 기존 레코드만 업데이트, 없을 때만 새 레코드 생성으로 수정.
- 현재 상태: **수정 완료. 빌드 정상.**
- 관련 커밋: c0585b4
