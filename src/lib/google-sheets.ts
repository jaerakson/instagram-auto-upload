...
export class GoogleSheetsService {
  ...
  async getSettings(): Promise<AppSettings> {
    ...
    return {
      ...
      geminiKeyNames: settingsMap.get('geminiKeyNames') || ',,,,,',
      defaultGeminiKeyIndex: Number(settingsMap.get('defaultGeminiKeyIndex') ?? -1),
  ...
  async updateSettings(settings: Partial<AppSettings>): Promise<void> {
    ...
    const rows: string[][] = [
      ...
      ['geminiKeyNames', merged.geminiKeyNames || ',,,,'],
      ['defaultGeminiKeyIndex', String(merged.defaultGeminiKeyIndex ?? -1)],
